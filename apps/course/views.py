...
#     # buyed = CourseOrder.objects.filter(course=course, buyer=request.user, status=2).exists()  #是否已经购买
#     context = {
#         'course': course,
#         # 'buyed': buyed
#     }
...
#
#     # course_id = request.GET.get('course_id')  # 获取当前课程id
#     # if not CourseOrder.objects.filter(course_id=course_id,buyer=request.user,status=2).exists():  #如果不存在
...
#         'order': order,
#         # /course/notify_url/
#         # request.build_absolute_uri：返回完整的域名
#         'notify_url': request.build_absolute_uri(reverse('course:notify_view')),    #回调
